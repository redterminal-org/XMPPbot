...
async def test_database_manager_execute_fetch(tmp_db_path):
    ...
    await db.execute("INSERT INTO rooms (room_jid, nick) VALUES (?, ?)", ("testroom@chat", "RoomBot"))
    # Fetch it back
    row = await db.fetch_one("SELECT * FROM rooms WHERE room_jid = ?", ("testroom@chat",))
...
async def test_database_manager_flush(tmp_db_path):
    ...
    await db.flush()
    row = await db.fetch_one("SELECT * FROM users WHERE jid=?", ("jid1@example.com",))
...
async def test_database_manager_close_flushes(tmp_db_path):
    ...
    async with aiosqlite.connect(tmp_db_path) as check_db:
        ...
        row = await check_db.execute("SELECT * FROM users WHERE jid=?", ("jid2@example.com",))

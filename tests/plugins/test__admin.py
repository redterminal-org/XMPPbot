...
async def test_bot_restart(monkeypatch, fake_bot, tmp_path):
    ...
    class FakeDB:
        async def close(self):
            db_closed.append("close")
            return None
        path = "/tmp/foo"
    fake_bot.db = FakeDB()
    monkeypatch.setattr(_admin.os, "execvp",
                        lambda exe, args: execvp_args.append(
                                          (exe, args))
                        or (_ for _ in ()).throw(SystemExit))
    notification_path = tmp_path / "notefile.json"
    monkeypatch.setattr(_admin, "RESTART_NOTIFICATION_FILE",
                        str(notification_path))
    monkeypatch.setattr(_admin, "json", types.SimpleNamespace(
        dump=lambda data, f: file_json.update(data)))
...
async def test_bot_restart_handles_file_write_fail(monkeypatch, fake_bot):
    ...
    class FakeDB:
        ...
        path = "/tmp/foo"
    fake_bot.db = FakeDB()
    monkeypatch.setattr(_admin.os, "execvp", lambda exe,
                        args: (_ for _ in ()).throw(SystemExit))

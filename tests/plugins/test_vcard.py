...
def msg(from_jid="room@x/resource", resource=None, type_="chat", to_jid="bot@domain"):
    if "/" in from_jid:
...
def patch_get_vcard(monkeypatch):
    class DummyVcard:
        def get(self, key):
            if key == "FN": return "Test User"
            if key == "BDAY": return "2001-01-01"
            if key == "ADR": return {"LOCALITY":"Loc","REGION":"Reg","CTRY":"CT"}
            return None
        xml = []
...
async def test_set_timezone_command(fake_bot, monkeypatch):
    ...
    fake_bot.db.users.plugin = lambda p: SimpleNamespace(get_global=get_global, set=set)
    ...
    found = any("TIMEZONE set to" in t[0] for t in getattr(fake_bot,"_replies",[]))
    assert found
...
async def test_set_timezone_invalid(fake_bot, monkeypatch):
    ...
    assert any("Invalid timezone" in t[0] for t in getattr(fake_bot,"_replies",[]))
...
async def test_field_cmds(fake_bot, monkeypatch, cmd, args, label, expect):
    ...
    m["type"] = "chat"
    # Patch plugin store so bot.db.users.plugin("vcard").get_global works even if key unused
    fake_bot.db.users.plugin = lambda plugin: SimpleNamespace(get_global=lambda k, d=None: {"room@x":True})
    await cmd(fake_bot, "s", "n", args, m, True)
    # Accept warning cases: some vcard plugins will warn about missing nicks if the minimal nick is not found
    expected_found = any(expect.lower() in x[0].lower() or label.lower() in x[0].lower() for x in getattr(fake_bot,"_replies",[]))
    # Also accept a warning reply about the nick not being found for negative coverage
    warning_found = any("not found in this room" in x[0].lower() for x in getattr(fake_bot,"_replies",[]))

...
async def test_parse_nick_and_message_basic():
    ...
    assert tell.parse_nick_and_message("  Some Name :  msg value ") == ("Some Name", "msg value")
...
async def test_tell_cmd_toggle(monkeypatch):
    ...
    # Simulate handled
    monkeypatch.setattr(tell, "handle_room_toggle_command", AsyncMock(return_value=True))
    # Simulate enabled room
    store.get_global.return_value = {"testroom": True}
    msg = {"from": MagicMock(bare="testroom"), "body": ",tell on", "type": "groupchat"}
...
async def test_tell_cmd_dm_and_args(monkeypatch):
    ...
    msg = {"from": MagicMock(bare="abc@conf"), "body": ",tell foo: hi", "type": "chat"}
    await tell.tell_cmd(bot, "jid", "nick", ["foo:", "hi"], msg, False)
    assert bot.reply.call_args[0][1].startswith("This command is only available in groupchats.")
...
async def test_tell_cmd_user_not_found(monkeypatch):
    ...
    msg["mucnick"] = "sender"
    monkeypatch.setattr(tell, "parse_nick_and_message", lambda v: ("someone", "test"))
    monkeypatch.setattr(tell, "get_jids_from_nick_index", AsyncMock(return_value=[]))
    monkeypatch.setattr("plugins._core.get_jids_from_nick_index", AsyncMock(return_value=[]))
...
async def test_tell_cmd_store(monkeypatch):
    ...
    monkeypatch.setattr(tell, "parse_nick_and_message", lambda v: ("other", "test-m"))
    monkeypatch.setattr(tell, "get_jids_from_nick_index", AsyncMock(side_effect=[["jidB"], ["jidA"]]))
    monkeypatch.setattr("plugins._core.get_jids_from_nick_index", AsyncMock(side_effect=[["jidB"], ["jidA"]]))
...
async def test_deliver_tell_messages_single(monkeypatch):
    ...
    store = AsyncMock()
    bot.db.users.plugin.return_value = store
    monkeypatch.setattr(tell, "get_jids_from_nick_index", AsyncMock(return_value=["jid1"]))
    monkeypatch.setattr("plugins._core.get_jids_from_nick_index", AsyncMock(return_value=["jid1"]))
    payload = {
        "recv_jid": "jid1",
        "send_jid": "jidX",
        "send_nick": "Envsi",
    ...
    monkeypatch.setattr(tell, "get_user_tzinfo", AsyncMock(return_value=pytz.timezone("UTC")))
    msg = {
        "from": MagicMock(bare="room@room"),
        "muc": {"nick": "Alpha"},
    }
    ...
    assert payload["send_nick"] in last_msg and payload["recv_nick"] in last_msg and payload["message"] in last_msg
...
async def test_deliver_tell_messages_no_messages(monkeypatch):
    ...
    bot = MagicMock()
    monkeypatch.setattr(tell, "get_jids_from_nick_index", AsyncMock(return_value=["jid"]))
    monkeypatch.setattr("plugins._core.get_jids_from_nick_index", AsyncMock(return_value=["jid"]))
...
def test_on_load_registers_event(monkeypatch):
    ...
    with patch("plugins.tell.partial", lambda fn, arg: (fn, arg)) as p:
        tell.on_load(bot)
        bot.bot_plugins.register_event.assert_called()
        event_name, event_type, event_func = bot.bot_plugins.register_event.call_args[0]

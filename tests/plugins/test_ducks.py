...
class DummyMsg:
    ...
    def __getitem__(self, key): return self.from_ if key=="from" else self.type
    def get(self, key, default=None):
        if key == "from": return self.from_
        if key == "type": return self.type
        if key == "body": return "dummy body"
        if key == "mucnick": return self.mucnick
        return default
...
class DummyStore:
    ...
    async def get_global(self, key, default=None): return self._globals.get(key, default)
    async def set_global(self, key, value): self._globals[key] = value
    async def get(self, jid, field=None):
        if field: return self._user.get(jid, {}).get(field)
        return self._user.get(jid)
    async def set(self, jid, field, value): self._user.setdefault(jid, {})[field] = value

class DummyBot:
    def __init__(self, user_stats=None, global_index=None):
        self._replies = []
        self.db = types.SimpleNamespace(users=types.SimpleNamespace(plugin=lambda name: DummyStore(user_stats, global_index)))
        self.reply = lambda msg, text, *a, **k: self._replies.append((text, msg))
...
async def test_duck_command_friends_and_enemies(monkeypatch):
    global_index = {"room@conf": {"jid/alice": {"display_name": "Alice", "befriended": 2, "trapped": 3}}}
...
async def test_duck_command_stats_path(monkeypatch):
    ...
    await ducks.duck_command(bot, "jid", "nick", ["stats", "jid/alice"], msg, True)
    assert any("befriend" in t[0].lower() or "no duck stats found" in t[0].lower() for t in bot._replies)
...
async def test_bef_trap_disabled_and_not_public(monkeypatch):
    ...
    async def fake_jid(*a, **k): return ("jid/alice", None, None)
    async def ensure_user(*a, **k): return None
    patch_ducks(monkeypatch, _is_public_muc=false_func, _is_enabled_for_room=false_async)
    await ducks.bef_command(bot, "jid", "nick", [], msg, True)
    await ducks.trap_command(bot, "jid", "nick", [], msg, True)
...
async def test_duckstats_command(monkeypatch):
    ...
    assert "befriend" in out or "no duck stats found" in out or "could not determine" in out
...
async def test_handle_duck_action_jid_fail(monkeypatch):
    ...
    patch_ducks(monkeypatch, get_real_jid=fake_get_real_jid, _ensure_user_exists=fake_get_real_jid)
...
async def test_handle_duck_action_no_duck(monkeypatch):
    ...
    patch_ducks(monkeypatch, get_real_jid=fake_get_real_jid, _ensure_user_exists=fake_get_real_jid)
    await ducks._handle_duck_action(bot, msg, "befriended")
    assert "no duck" in bot._replies[-1][0].lower() or "❌" in bot._replies[-1][0]
...
async def test_handle_duck_action_success(monkeypatch):
    ...
    async def fake_get_real_jid(bot, msg):
        return ("jid/alice", None, None)
    async def ensure_user(*a, **k): return None
    patch_ducks(monkeypatch, get_real_jid=fake_get_real_jid, _ensure_user_exists=ensure_user)
    await ducks._handle_duck_action(bot, msg, "befriended")
...
async def test_on_message_skips(monkeypatch):
    ...
    msg3 = DummyMsg()
    msg3.get = lambda key, default=None: "jid/testbot" if key == "from" else "test"
...
async def test_on_load_and_unload(monkeypatch):
    bot = DummyBot()
    spy = {}
    bot.bot_plugins.register_event = lambda *a, **k: spy.setdefault("registered", True)
    ...
    obj1 = CancelObj(); obj2 = CancelObj()
    ducks.SPAWN_TASKS["abc"] = obj1; ducks.EXPIRE_TASKS["abc"] = obj2

...
def test_read_until_delimiter():
    assert sed.read_until_delimiter("foo/bar/flags", "/") == ("foo", "bar/flags")
    assert sed.read_until_delimiter("foo\\/bar/flags", "/", True) == ("foo/bar", "flags")
...
def test_get_last_message_and_get_message_by_id_cache(monkeypatch):
    ...
    room = "room1"
    with patch.object(sed._core, "get_last_cached_message", return_value={"body": "hello"}):
        assert sed.get_last_message(room) == "hello"
    with patch.object(sed._core, "get_last_cached_message", return_value=None):
        assert sed.get_last_message(room) is None
    with patch.object(sed._core, "get_cached_message_by_id", return_value={"body": "foo"}):
        assert sed.get_message_by_id(room, "id123") == "foo"
    with patch.object(sed._core, "get_cached_message_by_id", return_value=None):
...
async def test_process_sed_correction_reply_and_edge(monkeypatch):
    ...
    is_room = True
    nick = "user1"
    patch_core = patch.object(sed._core, "extract_reply_quote", return_value="Hello foo foo")
    patch_msg_by_id = patch.object(sed, "get_message_by_id", return_value="Hello foo foo")
    patch_last_message = patch.object(sed, "get_last_message", return_value="Hello foo foo")

    # normal flow with quote
    with patch_core, patch_msg_by_id, patch_last_message:
        await sed.process_sed_correction(bot, nick, msg, is_room, "foo", "bar", "g")
    ...
    with patch.object(sed._core, "extract_reply_quote", return_value=None), \
         ...
         patch.object(sed, "get_last_message", return_value="no match here"):
        await sed.process_sed_correction(bot, nick, msg, is_room, "foo", "bar", "g")
        called_args = bot.reply.call_args[0]
    ...
    with patch.object(sed, "apply_sed", return_value=(None, 0)):
        await sed.process_sed_correction(bot, nick, msg, is_room, "(", "b", "")
        called_args = bot.reply.call_args[0]
        assert "Regex error" in called_args[1]

    # regex timeout
    with patch.object(sed, "apply_sed", return_value=(None, -1)):
        await sed.process_sed_correction(bot, nick, msg, is_room, "foo", "bar", "")
        called_args = bot.reply.call_args[0]
...
async def test_cmd_sed_handler_toggle_and_usage(monkeypatch):
    ...
    with patch.object(sed._core, "handle_room_toggle_command", AsyncMock(return_value=True)):
        await sed.cmd_sed_handler(bot, "sender", "nick", args, msg, is_room)
        assert not bot.reply.called  # toggle swallows response

    # Real sed parse
    with patch.object(sed._core, "handle_room_toggle_command", AsyncMock(return_value=False)), \
         patch.object(sed, "parse_prefixed_sed_command", return_value=("foo", "bar", "")), \
         patch.object(sed, "process_sed_correction", AsyncMock()):
        await sed.cmd_sed_handler(bot, "sender", "nick", args, msg, is_room)

    # Usage error
    with patch.object(sed._core, "handle_room_toggle_command", AsyncMock(return_value=False)), \
         patch.object(sed, "parse_prefixed_sed_command", return_value=(None, None, None)):
        await sed.cmd_sed_handler(bot, "sender", "nick", args, msg, is_room)
        assert bot.reply.called
...
async def test_on_message_async(monkeypatch):
    ...
    with \
        ...
        patch.object(sed, "process_sed_correction", AsyncMock()) as process_correction_mock, \
        patch.object(sed, "parse_any_sed_command", return_value=("foo", "bar", "")), \
        patch.object(sed._core, "cache_message"), \
        patch.object(sed._core, "JOINED_ROOMS", {room_name: {"nick": "botnick"}}):
    ...
    with patch.object(sed._core, "get_stanza_id", return_value="id10"):
        bot.boundjid = "botjid@muc"
        await sed.on_message(bot, msg2)  # Should return immediately, no crash and no call to process

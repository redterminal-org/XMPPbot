...
class DummyBot:
    def __init__(self, *, version="99.99-x", prefix=",", role=Role.ADMIN, plugins=None):
        self.replies = []
        self.version = version
        self.prefix = prefix
        self._role = role
        self.db = SimpleNamespace(users=SimpleNamespace(plugin=lambda name: AsyncMockStore()), roles={})
...
class DummyMsg:
    ...
    def __init__(self, body, is_room=True, room_jid="room@conf.test", nick="Bob"):
        self.msg = {
            "body": body,
...
class AsyncMockStore:
    async def get_global(self, key, default=None):
        ...
        return default
    async def set_global(self, key, value): pass

@pytest.fixture
def basic_plugins_and_commands(monkeypatch):
    def foo_handler(*a, **k): """Foo command docstring\nSecond line."""; return None
    def bar_handler(*a, **k): """Bar command admin only doc."""; return None
    def help_handler(*a, **k): """Help docstring."""; return None

    foo_cmd = Command(name="foo", handler=foo_handler, role=Role.USER, aliases=["fooz"])
    ...
    plugins = {
        ...
        "_hidden": SimpleNamespace(__doc__="Should hide for non-admin", __name__="_hidden"),
...
def flatten_lines(reply):
    ...
    if isinstance(reply, (list, tuple)):
        return "\n".join(str(l) for l in reply)
    return str(reply)

# ----- Tests -----
@pytest.mark.asyncio
async def test_general_help_lists_plugins_and_commands(basic_plugins_and_commands, monkeypatch):
...
@pytest.mark.asyncio
async def test_help_filters_hidden_and_no_cmd_plugins_for_user(basic_plugins_and_commands, monkeypatch):
    plugins, reg = basic_plugins_and_commands
    ...
    # Should only show foo (has user command), not bar (admin-only, hidden for normal users)
    assert "foo" in reply
...
async def test_command_help_notfound(basic_plugins_and_commands):
    ...
    await help_plugin.cmd_help(bot, "user@host", "Test", [",notfound"], msg, True)
    reply = flatten_lines(bot.replies[-1])
...
@pytest.mark.asyncio
async def test_plugin_help_no_permission_for_internal(basic_plugins_and_commands):
    plugins, reg = basic_plugins_and_commands
    bot = DummyBot(plugins=plugins, role=Role.USER)
    msg = DummyMsg(body=",help _hidden")
    await help_plugin.cmd_help(bot, "user@host", "Test", ["_hidden"], msg, True)
    reply = flatten_lines(bot.replies[-1])
    assert "unknown plugin" in reply.lower()  # Hides internal plugins for non-admin
...
async def test_inroom_help_toggle_invokes_toggler(monkeypatch):
    ...
    await help_plugin.help_inroom_command(bot, "jid", "nick", ["on"], msg, True)
...
async def test_inroom_help_usage_when_not_handled(monkeypatch):
    ...
    await help_plugin.help_inroom_command(bot, "jid", "nick", ["notareal"], msg, True)
    reply = flatten_lines(bot.replies[-1])

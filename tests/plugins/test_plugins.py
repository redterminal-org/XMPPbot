...
async def test_plugin_list(bot, msg):
    await plugins_module.plugin_list(bot, "adminjid", "AdminNick", [], msg, False)
...
async def test_plugin_info_existing(bot, msg):
    await plugins_module.plugin_info(bot, "adminjid", "AdminNick", ["plugins"], msg, False)
    out = bot.reply.call_args[0][1]
...
async def test_plugin_info_notfound(bot, msg):
    await plugins_module.plugin_info(bot, "adminjid", "AdminNick", ["notaplug"], msg, False)
    assert bot.reply.called
    assert "notaplug" in bot.reply.call_args[0][1]  # "Plugin 'notaplug' not found."

@pytest.mark.asyncio
async def test_plugin_info_missing_args(bot, msg):
    await plugins_module.plugin_info(bot, "adminjid", "AdminNick", [], msg, False)
    assert "Usage:" in bot.reply.call_args[0][1]

@pytest.mark.asyncio
async def test_plugin_load_single_success(bot, msg):
    await plugins_module.plugin_load(bot, "adminjid", "AdminNick", ["info"], msg, False)
    # load() called
...
async def test_plugin_load_all(bot, msg):
    await plugins_module.plugin_load(bot, "adminjid", "AdminNick", ["all"], msg, False)
...
async def test_plugin_load_error(bot, msg):
    ...
    await plugins_module.plugin_load(bot, "adminjid", "AdminNick", ["bad"], msg, False)
    assert "Error loading 'bad':" in bot.reply.call_args[0][1]

@pytest.mark.asyncio
async def test_plugin_load_missing_args(bot, msg):
    await plugins_module.plugin_load(bot, "adminjid", "AdminNick", [], msg, False)
    assert "Usage:" in bot.reply.call_args[0][1]

@pytest.mark.asyncio
async def test_plugin_unload_success(bot, msg):
    await plugins_module.plugin_unload(bot, "adminjid", "AdminNick", ["info"], msg, False)
...
async def test_plugin_unload_force(bot, msg):
    await plugins_module.plugin_unload(bot, "adminjid", "AdminNick", ["info", "force"], msg, False)
...
async def test_plugin_unload_no_plugins(bot, msg):
    await plugins_module.plugin_unload(bot, "adminjid", "AdminNick", ["plugins"], msg, False)
    assert "Cannot unload plugin manager" in bot.reply.call_args[0][1]

@pytest.mark.asyncio
async def test_plugin_unload_missing_args(bot, msg):
    await plugins_module.plugin_unload(bot, "adminjid", "AdminNick", [], msg, False)
    assert "Usage:" in bot.reply.call_args[0][1]

@pytest.mark.asyncio
async def test_plugin_reload_single(bot, msg):
    await plugins_module.plugin_reload(bot, "adminjid", "AdminNick", ["info"], msg, False)
...
async def test_plugin_reload_auto_flag(bot, msg):
    await plugins_module.plugin_reload(bot, "adminjid", "AdminNick", ["info", "auto"], msg, False)
    bot.bot_plugins.reload.assert_awaited_with("info", auto=True)
    assert "reloaded" in bot.reply.call_args[0][1]

@pytest.mark.asyncio
async def test_plugin_reload_missing_args(bot, msg):
    await plugins_module.plugin_reload(bot, "adminjid", "AdminNick", [], msg, False)
    assert "Usage:" in bot.reply.call_args[0][1]
...
async def test_plugin_reload_all_success(bot, msg):
    ...
    await plugins_module.plugin_reload(bot, "adminjid", "AdminNick", ["all"], msg, False)
    assert "plugins" in bot.bot_plugins.reload.call_args[0] or bot.bot_plugins.reload.call_args_list[0]
...
async def test_plugin_reload_all_auto_some_errors(bot, msg):
    ...
    await plugins_module.plugin_reload(bot, "adminjid", "AdminNick", ["all", "auto"], msg, False)
    out = bot.reply.call_args[0][1]

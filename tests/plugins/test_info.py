...
class DummyAioSession:
    ...
    def get(self, url, timeout=None):
        ...
        return list(self.resp_map.values())[0] if self.resp_map else AsyncContextResp(500, {})
...
def dummy_bot(monkeypatch):
    ...
    class DummyPlugin:
        def __init__(self):
            self._data = {}
        async def get_global(self, key, default=None): return self._data.get(key, default)
        async def set_global(self, key, value): self._data[key] = value
    dummy_plugin = DummyPlugin()
    bot.db.users.plugin = lambda plugin_name: dummy_plugin
    monkeypatch.setattr(info_plugin, "get_info_store", lambda bot: dummy_plugin)
    return bot
...
async def test_udict_usage(dummy_bot, fake_room_msg):
    await info_plugin.udict_search(dummy_bot, "jid", "nick", [], fake_room_msg, True)
...
async def test_udict_flow_found(monkeypatch, dummy_bot, fake_room_msg):
    ...
    monkeypatch.setattr(info_plugin.aiohttp, "ClientSession", lambda: DummyAioSession({"udict": resp}))
    dummy_bot.reset()
    await info_plugin.udict_search(dummy_bot, "jid", "nick", ["test"], fake_room_msg, True)
...
async def test_udict_error(monkeypatch, dummy_bot, fake_room_msg):
    ...
    monkeypatch.setattr(info_plugin.aiohttp, "ClientSession", lambda: BrokenSession())
    dummy_bot.reset()
    await info_plugin.udict_search(dummy_bot, "jid", "nick", ["fail"], fake_room_msg, True)
    text = "\n".join(map(str, dummy_bot.replies))
...
async def test_fediverse_usage(dummy_bot, fake_room_msg):
    await info_plugin.fediverse_latest(dummy_bot, "jid", "nick", [], fake_room_msg, True)
    text = "\n".join(str(x) for x in dummy_bot.replies)
    assert "Usage:" in text

@pytest.mark.asyncio
async def test_fediverse_invalid_format(dummy_bot, fake_room_msg):
    await info_plugin.fediverse_latest(dummy_bot, "jid", "nick", ["invalid"], fake_room_msg, True)
    text = "\n".join(str(x) for x in dummy_bot.replies)
...
async def test_fediverse_error(monkeypatch, dummy_bot, fake_room_msg):
    ...
    monkeypatch.setattr(info_plugin.aiohttp, "ClientSession", lambda: BrokenSession())
    dummy_bot.reset()
    await info_plugin.fediverse_latest(dummy_bot, "jid", "nick", ["@foo@bar"], fake_room_msg, True)
    text = "\n".join(str(x) for x in dummy_bot.replies)
...
async def test_fediverse_nomatches(monkeypatch, dummy_bot, fake_room_msg):
    ...
    monkeypatch.setattr(
        ...
        lambda: DummyAioSession({"lookup": resp_user, "/statuses": resp_timeline}),
    )
    dummy_bot.reset()
    await info_plugin.fediverse_latest(dummy_bot, "jid", "nick", ["@someone@host"], fake_room_msg, True)
...
async def test_fediverse_success(monkeypatch, dummy_bot, fake_room_msg):
    timeline_content = [{
        ...
        "url": "u", "reblogs_count": 1, "replies_count": 2, "favourites_count": 3
    }]
    resp_user = AsyncContextResp(200, {"id": "42"})
...
async def test_acronyms_cmd_found(tmp_slang_files, dummy_bot, fake_room_msg):
    ...
    dummy_bot.reset()
    await info_plugin.acronyms_cmd(dummy_bot, "jid", "nick", ["lgtm"], fake_room_msg, True)
    text = "\n".join(str(x) for x in dummy_bot.replies)
    assert "LGTM:" in text

@pytest.mark.asyncio
async def test_acronyms_cmd_not_found(tmp_slang_files, dummy_bot, fake_room_msg):
    main, _, _ = tmp_slang_files
    with main.open("w", encoding="utf-8") as f: pass
    dummy_bot.reset()
    await info_plugin.acronyms_cmd(dummy_bot, "jid", "nick", ["NOSUCH"], fake_room_msg, True)
    out = "\n".join(str(x) for x in dummy_bot.replies)
...
async def test_acronyms_add_cmd(tmp_slang_files, dummy_bot, fake_room_msg):
    ...
    dummy_bot.reset()
    await info_plugin.acronyms_add_cmd(dummy_bot, "jid", "nick", ["foo", "Bar baz"], fake_room_msg, True)
    out = "\n".join(str(x) for x in dummy_bot.replies)
...
async def test_acronyms_remove_cmd(tmp_slang_files, dummy_bot, fake_room_msg):
    ...
    dummy_bot.reset()
    await info_plugin.acronyms_remove_cmd(dummy_bot, "jid", "nick", ["foo", "to remove"], fake_room_msg, True)
    out = "\n".join(str(x) for x in dummy_bot.replies)
...
async def test_acronyms_list_cmd(tmp_slang_files, dummy_bot, fake_room_msg):
    ...
    with open(str(rem), "a", encoding="utf-8", newline="") as f:
        csv.writer(f).writerow(["foo2", "bar2", "nick1"])
    dummy_bot.reset()
    await info_plugin.acronyms_list_cmd(dummy_bot, "jid", "nick", [], fake_room_msg, True)
...
@pytest.mark.asyncio
async def test_acronyms_merge_and_delete(monkeypatch, dummy_bot, fake_room_msg, tmp_slang_files):
    ...
    dummy_bot.reset()
    await info_plugin.acronyms_merge_cmd(dummy_bot, "jid", "nick", [], fake_room_msg, True)
    messages = "\n".join(x[1] for x in dummy_bot.replies)
...
@pytest.mark.asyncio
async def test_acronyms_delete_by_desc_and_nick(tmp_slang_files, dummy_bot, fake_room_msg):
    _, add, rem = tmp_slang_files
    ...
    dummy_bot.reset()
    await info_plugin.acronyms_delete_cmd(dummy_bot, "jid", "nick", ["abcd", "Desc1"], fake_room_msg, True)
    await info_plugin.acronyms_delete_cmd(dummy_bot, "jid", "nick", ["def", "Desc3"], fake_room_msg, True)
    with open(str(add), encoding="utf-8") as f:
    ...
    await info_plugin.acronyms_delete_cmd(dummy_bot, "jid", "nick", ["nickB"], fake_room_msg, True)
...
async def test_wikipedia_usage(dummy_bot, fake_room_msg):
    dummy_bot.reset()
    await info_plugin.wikipedia_command(dummy_bot, "jid", "nick", [], fake_room_msg, True)
    text = "\n".join(str(x) for x in dummy_bot.replies)
    assert "usage" in text.lower()

@pytest.mark.asyncio
async def test_wikipedia_notfound(monkeypatch, dummy_bot, fake_room_msg):
    monkeypatch.setattr(info_plugin, "fetch_wikipedia_summary", lambda term: None)
    orig = asyncio.get_event_loop
    monkeypatch.setattr(info_plugin.asyncio, "get_event_loop", lambda: orig())
    await info_plugin.wikipedia_command(dummy_bot, "jid", "nick", ["somethingunreal"], fake_room_msg, True)
    text = "\n".join(str(x) for x in dummy_bot.replies)
    assert "No Wikipedia summary found" in text

@pytest.mark.asyncio
async def test_wikipedia_found(monkeypatch, dummy_bot, fake_room_msg):
    monkeypatch.setattr(info_plugin, "fetch_wikipedia_summary", lambda term: ("Python", "A summary", "http://wiki/Python"))
    orig = asyncio.get_event_loop
    monkeypatch.setattr(info_plugin.asyncio, "get_event_loop", lambda: orig())
    await info_plugin.wikipedia_command(dummy_bot, "jid", "nick", ["Python"], fake_room_msg, True)
    text = "\n".join(str(x) for x in dummy_bot.replies)
...
async def test_information_command_toggle_on(dummy_bot, fake_room_msg):
    await info_plugin.information_command(dummy_bot, "jid", "nick", [], fake_room_msg, True)
    text = "\n".join(str(x) for x in dummy_bot.replies)

...
async def test_update_vcard_import_error(monkeypatch, tmp_path):
    ...
    def our_spec_from_file_location(name, location):
        ...
        return spec
    monkeypatch.setattr(_reg_profile.importlib.util, "spec_from_file_location", our_spec_from_file_location)
    error_msgs = []
    monkeypatch.setattr(_reg_profile, "log", types.SimpleNamespace(
...
async def test_update_vcard_no_change(monkeypatch, tmp_path):
    vcard_py = tmp_path / "vcard.py"
    vcard_text = "<vCard><FN>bot</FN></vCard>"
...
async def test_update_vcard_success(monkeypatch, tmp_path):
    ...
    write_called = []
    monkeypatch.setattr(_reg_profile, "write_hash", lambda path, value: write_called.append((path, value)))
    class Bot:
        def make_iq_set(self):  # minimal stub for tests
            class IQ:
                def append(self, elem): self.elem = elem
                async def send(self): IQ.sent = True
            return IQ()
    info_msgs = []
    monkeypatch.setattr(_reg_profile, "log", types.SimpleNamespace(
        info=lambda msg: info_msgs.append(msg),
...
async def test_update_avatar_all_paths(monkeypatch, tmp_path):
    ...
    bot = {"xep_0084": types.SimpleNamespace(
            publish_avatar=lambda data: None,
            publish_avatar_metadata=lambda meta: None)}
    # No avatar_path
    monkeypatch.setattr(_reg_profile, "config", {})   # config with nothing
    await _reg_profile.update_avatar(bot)
    # Not exists
    monkeypatch.setattr(_reg_profile, "config", {"avatar": "missing.png", "avatar_type": "image/png"})
    monkeypatch.setattr(_reg_profile.os.path, "exists", lambda p: False)
    monkeypatch.setattr(_reg_profile, "log", types.SimpleNamespace(warning=lambda m: None))
    await _reg_profile.update_avatar(bot)
    ...
    def open_mock(path, mode="r", *a, **k):
        if "b" in mode:
            ...
            class DummyW:
                ...
                def write(self, val): self._written.append(val)
                def __enter__(self): return self
                def __exit__(self, *a): pass
            return DummyW()
    ...
    monkeypatch.setattr(_reg_profile, "read_hash", lambda p: "hash1")
    monkeypatch.setattr(_reg_profile, "log", types.SimpleNamespace(info=lambda m: None, error=lambda m: None))
    await _reg_profile.update_avatar(bot)
    # Exists, bad type
    monkeypatch.setattr(_reg_profile, "read_hash", lambda p: "different")
    monkeypatch.setattr(_reg_profile, "config", {"avatar": "avatar.png", "avatar_type": "bad/image"})
    await _reg_profile.update_avatar(bot)
    # Exists, good type, happy path
    monkeypatch.setattr(_reg_profile, "config", {"avatar": "avatar.png", "avatar_type": "image/png"})
    monkeypatch.setattr(_reg_profile, "sha1", lambda d: "newhash")
    monkeypatch.setattr(_reg_profile, "read_hash", lambda p: "oldhash")
    wrote = []
    monkeypatch.setattr(_reg_profile, "write_hash", lambda path, value: wrote.append((path, value)))
    metadata_published = []
    async def publish_avatar(data):
        metadata_published.append(data)
    async def publish_avatar_metadata(meta):
        metadata_published.append(meta)
    bot2 = {"xep_0084": types.SimpleNamespace(
        publish_avatar=publish_avatar,
        publish_avatar_metadata=publish_avatar_metadata
    )}
    await _reg_profile.update_avatar(bot2)
    assert wrote and metadata_published
...
async def test_setup_profile_user_entry(monkeypatch):
    ...
    got_roster_called = []
    class FakeDBUsers:
        async def get(self, jid): return {"jid": jid}
        async def create(self, jid, nick): assert False
    class FakeBot:
        async def get_roster(self): got_roster_called.append(True)
    ...
    monkeypatch.setattr(_reg_profile, "log", types.SimpleNamespace(info=lambda m: wrote.append(m), error=lambda m: None))
...
async def test_setup_profile_user_created(monkeypatch):
    ...
    called = []
    class FakeDBUsers:
        async def get(self, jid): return None
        async def create(self, jid, nick): called.append(("create", jid, nick))
    class FakeBot:
        async def get_roster(self): called.append("roster")
        boundjid = type("bjid", (), {"bare": "jidval"})()
        db = types.SimpleNamespace(users=FakeDBUsers())
    monkeypatch.setattr(_reg_profile, "update_vcard", lambda bot: _awaitable(None))
    monkeypatch.setattr(_reg_profile, "update_avatar", lambda bot: _awaitable(None))
    monkeypatch.setattr(_reg_profile, "log", types.SimpleNamespace(info=lambda m: called.append(m), error=lambda m: None))
...
async def test_setup_profile_user_create_error(monkeypatch):
    ...
    class FakeDBUsers:
        async def get(self, jid): return None
        async def create(self, jid, nick): raise Exception("fail!")
    class FakeBot:
        async def get_roster(self): called.append("roster")
        boundjid = type("bjid", (), {"bare": "jidval"})()
        db = types.SimpleNamespace(users=FakeDBUsers())
    monkeypatch.setattr(_reg_profile, "update_vcard", lambda bot: _awaitable(None))
    monkeypatch.setattr(_reg_profile, "update_avatar", lambda bot: _awaitable(None))
    monkeypatch.setattr(_reg_profile, "log", types.SimpleNamespace(info=lambda m: called.append(m), error=lambda m: called.append("error")))
    monkeypatch.setattr(_reg_profile, "config", {"nick": "nick"})
    await _reg_profile.setup_profile(FakeBot())
...
async def test_on_load_and_on_ready(monkeypatch):
    ...
    class DummyUsers:
        def plugin(self, name): return DummyStore()
    class Bot:
        def register_plugin(self, name): called.append(name)
    ...
    assert "xep_0054" in called and "xep_0163" in called and "xep_0084" in called
